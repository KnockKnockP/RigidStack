    ...
    public class NetworkRoomManager : NetworkManager {
        ...
        void SceneLoadedForPlayer(NetworkConnection conn, GameObject roomPlayer) {
            if (logger.LogEnabled())
                logger.LogFormat(LogType.Log, "NetworkRoom SceneLoadedForPlayer scene: {0} {1}", SceneManager.GetActiveScene().path, conn);
        ...
        public override void OnServerAddPlayer(NetworkConnection conn) {
            if (IsSceneActive(RoomScene)) {
                ...
                if (logger.LogEnabled())
                    logger.LogFormat(LogType.Log, "NetworkRoomManager.OnServerAddPlayer playerPrefab:{0}", roomPlayerPrefab.name);
